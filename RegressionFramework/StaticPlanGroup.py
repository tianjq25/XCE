...
class StaticPlanGroup:
    ...
    def get_group_key(self, plan):
        """
        方案一：遍历 Plan 树，收集所有 Scan 节点的表名，去重并排序。
        """
        tables = set()
        self._recurse_collect_tables(plan.root, tables)
        # 排序非常重要，确保 (A, B) 和 (B, A) 生成相同的 Key
        # print(" ".join(sorted(list(tables))))
        return " ".join(sorted(list(tables)))

    def _recurse_collect_tables(self, node, tables):
        # 1. 如果是扫描节点（只有扫描节点才直接对应物理表）
        ...
        for child in node.children:
            self._recurse_collect_tables(child, tables)
