...
class JoinPlanNode(PlanNode):
    def __init__(self, node_json, node_id):
        ...
        self.join_key = sorted(self.join_key)
...
class Plan:
    ...
    def __init__(self, plan_json, plan_id, predict=None):
        ...
        self.predict = self._get_predict()
    ...
    def _get_execution_time(self):
        if "Execution Time" in self.plan_json:
            return self.plan_json["Execution Time"]
        return 0

    def _get_metric(self):
        if "metric" in self.plan_json:
            return self.plan_json["metric"]
        return None

    def _get_id(self):
        if "id" in self.plan_json:
            return self.plan_json["id"]
        return None

    def _get_predict(self):
        if "predict" in self.plan_json:
            return self.plan_json["predict"]
        return None
