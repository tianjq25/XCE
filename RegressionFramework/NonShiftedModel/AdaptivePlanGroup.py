...
from RegressionFramework.NonShiftedModel.AdaptivaGroupAction import Action, FilterColAction, FilterOpAction, FilterValueAction, JoinKeyAction
...
from RegressionFramework.Plan.Plan import Plan, FilterPlanNode, JoinPlanNode
...
class AdaptivePlanGroup:
    ...
    def get_group_with_root(cls, root, plan: Plan):
        ...
        while not tree_node.is_leaf():
            ...
            # if plan.plan_id in action.plan_node_id:
            #     tree_node = tree_node.left_child
            # else:
            #     tree_node = tree_node.right_child
    ...
    def _split_leaf_node(self, tree_node: TreeNode, min_ele_count, root: TreeNode, plans_for_queries, structure_iter):
        ...
        plan_id_2_group = {}
        sorted_actions = sorted(tree_node.actions, key=lambda a: (type(a).__name__, a.plan_node_id))
        for i, action in enumerate(sorted_actions):
            ...
            if left_tree_node.size() < self.min_ele_count or right_tree_node.size() < self.min_ele_count:
                continue
            # if left_tree_node.size() < 2 or right_tree_node.size() < 2:
            #     continue
            score = self._score(root, tree_node, left_tree_node, right_tree_node, action, plans_for_queries, self.thres,
    ...
    def _score(self, root: TreeNode, cur_tree_node: TreeNode, left_tree_node: TreeNode, right_tree_node: TreeNode,
               action: Action, plans_for_queries, thres, structure_iter, plan_id_2_group):
        score = 0
...
class AdaptivePlanGroupOptimization(AdaptivePlanGroup):
    ...
    def __init__(self, delta=0.001, min_ele_count=20, static_config=None, thres=0.6):
        super().__init__(delta, min_ele_count, static_config, thres)

    def _score(self, root: TreeNode, cur_tree_node: TreeNode, left_tree_node: TreeNode, right_tree_node: TreeNode,
               action: Action, plans_for_queries, thres, structure_iter, plan_id_2_group):
        return self.score_computer.score(root, cur_tree_node, left_tree_node, right_tree_node, action,
                                         plans_for_queries, thres, structure_iter, plan_id_2_group)
...
class ScoreComputer:
    ...
    def score(self, root: TreeNode, cur_tree_node: TreeNode, left_tree_node: TreeNode, right_tree_node: TreeNode,
        ...
        # lambda
        return sum(select_times_for_query) * 1
