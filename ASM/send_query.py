...
def send_query(dataset, method_name, query_file, save_folder, get_plan, iteration=None):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--get_plan', type=bool, default=False, help="Whether store the plan")

    args = parser.parse_args()

    if args.iteration:
        for i in range(args.iteration):
            send_query(args.dataset, args.method_name, args.query_file, args.save_folder, args.get_plan, i+1)
    else:
        send_query(args.dataset, args.method_name, args.query_file, args.save_folder, args.get_plan, None)