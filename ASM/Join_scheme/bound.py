...
from ASM.Join_scheme.join_graph import get_join_hyper_graph, parse_query_all_join, dfs, get_sub_query_equivalent_group
from ASM.Join_scheme.data_prepare import identify_key_values
...
class Bound_ensemble:
    ...
    def get_all_id_conditional_distribution(self, tables_alias, join_keys, get_Probability=None):
        t = time.time()
        nc_factors, m_time = initialize_model(self.ncs, tables_alias, join_keys, self.schema, sample_size=self.sample_size, data=self.query_predicate, get_Probability=get_Probability)
    ...
    def initialize_sample(self, factors, tables_all, equivalent_group, table_key_group_map, get_Probability=None):
        t = time.time()
        ...
        for order, PK in enumerate(self.query_dfs_PKs):
            ...
            for i in range(self.num_fact_cols[PK]):
                ...
                for key in equivalent_group[PK]:
                    ...
                    if get_Probability is not None:
                        with open(get_Probability, 'a') as f:
                            print(f"first get_P: {table}", file=f)
                    P, factor.sample, factor.logits, returned_idx = self.ncs[table].get_P(factor.cur_idx, idx, self.sample_size, factor.sample, factor.logits,
                                                                                          factor.cols, factor.ops, factor.vals, factor.dom_ops, get_Probability=get_Probability
                                                                                          )
        ...
        for alias in factors:
            ...
            for idx in range(factor.cur_idx, len(factor.cols)):
                ...
                if factor.ops[idx] is not None:
                    ...
                    if get_Probability is not None:
                        with open(get_Probability, 'a') as f:
                            print(f"second get_P: {table}", file=f)
    ...
    def get_cardinality_bound_all(self, query_str, sub_plan_query_str_all, get_Probability=None, optimizer=None):
        """
        ...
        conditional_factors, initialize_model_query_convert_time, initialize_model_time = self.get_all_id_conditional_distribution(tables_all, join_keys, get_Probability=get_Probability)
        initialize_model_query_convert_time += query_convert_time

        if not no_PE:
            # print(conditional_factors, tables_all, equivalent_group, table_key_group_map)
            initialize_sample_time = self.initialize_sample(conditional_factors, tables_all, equivalent_group, table_key_group_map, get_Probability=get_Probability)
        else:
        ...
        if optimizer is not None:
            non_optimized_cardinality_bounds = []
            # optimized_conditional_factors = dict()
            optimized_cache_sub_queries = dict()
            optimized_conditional_factors = copy.deepcopy(conditional_factors)
            tables, filters, joins = optimizer[2].parse(query_str)

            for alias in tables:
                tmp_table = {alias: tables[alias]}
                tmp_filter = {alias: filters.get(alias, [])}
                tmp_join = []
                tmp_sql = optimizer[2].reconstruct_sql(tmp_table, tmp_filter, tmp_join)
                plan = optimizer[1].convert_sql_to_plan_without_execute(tmp_sql)
                plan_for_query = optimizer[0]._to_plan_object(json_str_to_json_obj(plan[0]), 0, None)

                # repair_value = optimizer[0].evaluate(plan_for_query)
                if len(optimizer) == 3:
                    repair_value = optimizer[0].evaluate(plan_for_query)
                else:
                    class_label = optimizer[0].evaluate(plan_for_query)
                    if class_label == -1:
                        repair_value = -1
                    elif class_label == 1:
                        repair_value = optimizer[3].evaluate(plan_for_query)
                    elif class_label == 0:
                        # repair_value = optimizer[4].evaluate(plan_for_query)
                        repair_value = -1

                if repair_value != -1:
                    # optimized_conditional_factors[alias].densities = optimized_conditional_factors[alias].densities * repair_value
                    if math.exp(repair_value) > 2:
                        optimized_conditional_factors[alias].densities = optimized_conditional_factors[alias].densities * 2
                    else:
                        optimized_conditional_factors[alias].densities = optimized_conditional_factors[alias].densities * math.exp(repair_value)
        ...
        for i, (left_tables, right_tables) in enumerate(sub_plan_query_str_all):
            ...
            right_tables = right_tables.strip()
            ...
            if " " in right_tables:
                ...
                for (t, key_str) in decompositions:
                    ...
                    try:
                        if optimizer is not None:
                            tmp_join_tables = {tmp_t:tables[tmp_t] for tmp_t in all_tables}
                            tmp_join_filters = {tmp_t:filters[tmp_t] for tmp_t in all_tables}
                            UnionFind_obj = UnionFind()

                            tmp_join_condions = []
                            # print(join_cond[all_join_tables[0]], join_cond[all_join_tables[-1]])
                            for _idx, tmp_t in enumerate(all_tables):
                                # print(tmp_t, join_cond[tmp_t])
                                for cond in join_cond[tmp_t]:
                                    left_cond, right_cond = cond.split('=')
                                    left_cond = left_cond.strip()
                                    right_cond = right_cond.strip()
                                    tmp_left_table = left_cond.split('.')[0]
                                    tmp_right_table = right_cond.split('.')[0]
                                    if (tmp_left_table == tmp_t and tmp_right_table in all_tables[_idx + 1:]) or (tmp_right_table == tmp_t and tmp_left_table in all_tables[_idx + 1:]):
                            ...
                            join_sql = optimizer[2].reconstruct_sql(tmp_join_tables, tmp_join_filters, tmp_join_condions)
                            plan = optimizer[1].convert_sql_to_plan_without_execute(join_sql)
                            plan_for_query = optimizer[0]._to_plan_object(json_str_to_json_obj(plan[0]), 0, None)
                            ...
                            non_optimized_cardinality_bounds.append(max(res, 1))

                            if repair_value == -1:
                                key_factor = optimized_cache_sub_queries[key_str]
                                optimized_bound_factor, res = self.join_with_one_table_nc(sub_plan_query_str,
                                                                                                            t,
                                                                                                            tables_all,
                                                                                                            key_factor,
                                                                                                            optimized_conditional_factors[t],
                                                                                                            optimized_conditional_factors,
                                                                                                            table_equivalent_group,
                                                                                                            table_key_equivalent_group,
                                                                                                            table_key_group_map,
                                                                                                            join_cond
                                                                                                            )
                                # optimized_bound_factor = copy.deepcopy(curr_bound_factor)
                            else:
                                # res = res * repair_value
                                if math.exp(repair_value) > 2:
                                    res = res * 2
                                else:
                                    res = res * math.exp(repair_value)
                                optimized_bound_factor = NCFactor_Group(res)
                                optimized_bound_factor.join_cond = curr_bound_factor.join_cond
                                optimized_bound_factor.equivalent_groups = curr_bound_factor.equivalent_groups
                                optimized_bound_factor.table_key_equivalent_group = curr_bound_factor.table_key_equivalent_group
                        else:
                            curr_bound_factor, res = self.join_with_one_table_nc(sub_plan_query_str,
                                                                                t,
                                                                                tables_all,
                        ...
                        print(conditional_factors)
                        print(sub_plan_query_str_all)
                        print(left_tables, right_tables)
                ...
                if optimizer is not None:
                    ...
                    if repair_value == -1:
                        optimized_bound_factor, res = self.join_two_tables_nc(sub_plan_query_str,
                                                                                left_tables,
                                                                                right_tables,
                                                                                tables_all,
                                                                                optimized_conditional_factors,
            ...
            if optimizer is not None:
                optimized_cache_sub_queries[sub_plan_query_str] = optimized_bound_factor

            res = max(res, 1)
            cardinality_bounds.append(res)

        if optimizer is not None:
            assert len(non_optimized_cardinality_bounds) == len(cardinality_bounds), f"non_optimized_cardinality_bounds and cardinality_bounds have different lengths"
            return cardinality_bounds, non_optimized_cardinality_bounds, initialize_model_query_convert_time, initialize_model_time, initialize_sample_time
        else:
    ...
    def join_with_one_table_nc(self, sub_plan_query_str: str, left_table: str, tables_all: dict(), right_bound_factor: NCFactor_Group, cond_factor_left: NCFactor, conditional_factors,
                               table_equivalent_group, table_key_equivalent_group, table_key_group_map, join_cond, optimizer = None
                               ):
