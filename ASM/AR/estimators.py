...
import ASM.AR.common as common
import ASM.AR.datasets as datasets
import ASM.AR.distributions as distributions
import ASM.AR.join_utils as join_utils
import ASM.AR.made as made
import ASM.AR.utils as utils
...
class ProgressiveSampling(CardEst):
    ...
    def _infer_table(self,
        ...
        for i in col_range:
            ...
            if (not self.shortcircuit) or (operators[natural_idx] is not None):
                ...
                # =======================================================
                # [Modification 1]: Output the marginal selectivity of each column
                # =======================================================
                if get_Probability is not None:
                    if mask is not None:
                        # Compute the number of True values in the current column mask
                        # divided by the total number of rows
                        # .item() is used to convert the tensor to a Python float
                        col_selectivity = torch.sum(mask).item() / nrows
                        col_name = columns[natural_idx].name
                        with open(get_Probability, "a") as f:
                            print(f"[Column Selectivity] Column: {col_name}, Selectivity: {col_selectivity:.6f}", file=f)
                    else:
                        # If mask is None, it means this column has no filter condition,
                        # so the selectivity is 1.0
                        with open(get_Probability, "a") as f:
                            print(f"[Column Selectivity] Column: {columns[natural_idx].name}, Selectivity: 1.0", file=f)
                    # =======================================================
        ...
        if p is None:
            ...
            if get_Probability is not None:
                # =======================================================
                # [Modification 2]: Output the total selectivity of the entire table
                # =======================================================
                # At this point, p is already the final selectivity tensor
                total_selectivity = p.item()
                with open(get_Probability, "a") as f:
                    print(f"[Total Selectivity] Total Table Selectivity: {total_selectivity:.6f}", file=f)
                # =======================================================
    ...
    def _sample_n(self,
        ...
        for i in col_range:
            ...
            if self.shortcircuit and operators[natural_idx] is None:
                ...
                if self.model.output_encoding is not None or self.model.UseDMoL(natural_idx):
                    ...
                    if get_Probability is not None:
                        # ==================== [New code begins] ====================
                        # Get the current column name for easier inspection
                        curr_col_name = columns[natural_idx].name
                        
                        # Output the conditional probability of the current column
                        # Note: probs_i_summed is a tensor containing the probabilities
                        # of num_samples samples
                        # Here we print the average value
                        # (i.e., the average selectivity at this step) for easier observation
                        avg_prob = probs_i_summed.mean().item()
                        with open(get_Probability, "a") as f:
                            print(f"[Intermediate Probability] Column: {curr_col_name}, Average Probability P(Col|Context): {avg_prob:.6f}", file=f)
                        
                        # If you want to inspect the specific tensor values (the first 5 samples):
                        # print(f"    Probabilities of the first 5 samples: {probs_i_summed[:5].cpu().numpy()}")
                        # ==================== [New code ends] ====================
        ...
        if get_Probability is not None:
            # ==================== [New code begins] ====================
            # Output the final query probability P(Query)
            # Similarly, p is a [num_samples] tensor, and we usually care about
            # its mean value (i.e., the estimated selectivity)
            final_selectivity = p.mean().item()
            with open(get_Probability, "a") as f:
                print(f"========== [Final Result] Average Probability of P(Query): {final_selectivity:.10f} ==========", file=f)
            # ==================== [New code ends] ====================
    ...
    @profile
    def _infer_table_optimize(self,
                            ordering,
                            columns,
                            operators,
                            vals,
    ...
    def QueryTree(self, query: dict, sample_alg='ps', keys=None, sample_size=2048, use_raw_table=False, subvar_dropout=False, adjust_fact_col=False, get_Probability=None, optimizer = None):
        ...
        if optimizer is not None:
            with torch.no_grad():
                ...
                if num_orderings == 1:
                    ...
                    self.OnStart()
                    if use_raw_table:
                        p1, p2, m, cur_idx = self._infer_table_optimize(
                            inv_ordering,
                            columns,
                            operators,
                            vals,
                        ...
                        self.OnEnd()
                        return p1, p2, self.cardinality, cur_idx, None, None, m, columns, operators, vals, dominant_ops
                    else:
                        p1, p2, sample, logits1, logits2, cur_idx = self._sample_n_optimize(
                            sample_size,
                            inv_ordering,
                            columns,
                            operators,
                    ...
                    self.OnEnd()
                    return p1, p2, self.cardinality, cur_idx, sample.detach().clone(), logits1.detach().clone(), logits2.detach().clone(), columns, operators, vals, dominant_ops
        else:
            with torch.no_grad():
                # set sample to zero
                inp_buf = self.inp.zero_()
...
class FactorizedProgressiveSampling(ProgressiveSampling):
    ...
    def get_P(self, start_idx, return_idx, sample_size, sample, logits, cols, ops, vals, dom_ops, use_raw_table=False, subvar_dropout=False, adjust_fact_col=False, get_Probability=None):
