...
import ASM.AR.estimators as estimators_lib
from ASM.AR.estimators import use_ctypes
import ASM.AR.experiments as experiments
import ASM.AR.fair_sampler as fair_sampler
import ASM.AR.join_utils as join_utils
import ASM.AR.made as made
import ASM.AR.train_utils as train_utils
...
class NeuroCard(tune.Trainable):
    ...
    def get_P(self, start_idx, return_idx, sample_size, sample, logits, cols, ops, vals, dom_ops, get_Probability=None):
        for est in self.estimators:
            return est.get_P(start_idx, return_idx, sample_size, sample, logits, cols, ops, vals, dom_ops, use_raw_table=self.use_raw_table, subvar_dropout=self.subvar_dropout, adjust_fact_col=self.adjust_fact_col, get_Probability=get_Probability)
    ...
    def evaluate_one_tree(self, query, keys=None, sample_size=2048, get_Probability=None, optimizer = None):

        self.model.model_bits = 0

        for est in self.estimators:
            assert self.sample_alg == 'ps', self.sample_alg
            return est.QueryTree(query, sample_alg=self.sample_alg, keys=keys, sample_size=sample_size, use_raw_table=self.use_raw_table, subvar_dropout=self.subvar_dropout, adjust_fact_col=self.adjust_fact_col, get_Probability=get_Probability, optimizer = optimizer)
