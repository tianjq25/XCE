...
#include "pgm/storage_utility.hpp"
#include "pgm_index.hpp"
#include "defs.h"
#include <fcntl.h>    // For shm_open
#include <sys/mman.h> // For shared memory APIs
 ...
 */
    ...
    class DynamicPGMIndex {
        ...
        using Item =
                std::conditional_t<std::is_pointer_v<V> || std::is_arithmetic_v<V>, ItemA,
                        ItemB>;
        ...
        uint8_t
                min_level;  ///< Levels 0..min_level are combined into one large level.
        uint8_t min_index_level;  ///< Minimum level on which an index is constructed.
        uint8_t min_disk_level;
        size_t buffer_max_size;   ///< Size of the combined upper levels, i.e.
        ///< max_size(0) + ... + max_size(min_level).
        uint8_t used_levels;  ///< Equal to 1 + last level whose size is greater than
        ///< 0, or = min_level if no data.
        std::vector<Level>
                levels;  ///< (i-min_level)th element is the data array at the ith level.
        std::vector<PGMType>
                pgms;  ///< (i-min_index_level)th element is the index at the ith level.

        /// ***** for disk version *****///
        std::string suffix;
        std::string shm_name;
        // const char* shm_name = "/pgm_shm";
        StorageManager *sm;
        typedef struct {
            uint8_t base;
            uint8_t min_level;
    ...
    private:
        ...
        std::vector<int> data_file_handlers;
        std::vector<int> index_file_handlers;
        int metadata_handler;
        void *buf = aligned_alloc(BLOCK_SIZE, 100 * BLOCK_SIZE); // 实际上只用了一个块大小?
        int shm_fd;
        ...
        // 预取
        int cached_block_id0, cached_block_id1;
        void *cached_block0 = aligned_alloc(BLOCK_SIZE, BLOCK_SIZE); // left
        void *cached_block1 = aligned_alloc(BLOCK_SIZE, BLOCK_SIZE); // right

        void initial_metadata() {
            metadata.min_index_level = min_index_level;
            metadata.min_level = min_level;
            metadata.min_disk_level = min_disk_level;
            metadata.base = base;
        ...
        void load_metadata() {
            sm->read_block(metadata_handler, buf, 0);
            memcpy(&metadata, buf, MetadataSize);
            min_index_level = metadata.min_index_level;
            min_disk_level = metadata.min_disk_level;
        ...
        void flush_metadata() {
            memcpy(buf, &metadata, MetadataSize);
            sm->write_block(metadata_handler, buf, 0);
            return;
        }
        ...
        void create_related_empty_files() {
            for (auto i = 0; i < 12 - min_disk_level; i++) {
                // std::ostringstream oss;
                // oss << i << "." << suffix << "_data";
                std::string name = suffix + "_" + std::to_string(i) + "_data";
            ...
            for (auto i = 0; i < 12 - min_index_level; i++) {
                // std::ostringstream oss;
                // oss << i << "." << suffix << "_index";
                std::string name = suffix + "_" + std::to_string(i) + "_index";
                // FILE *fp = fopen(oss.str().c_str(), "wb");
                int fd = open(name.c_str(), O_RDWR | O_CREAT, 0644);
                close(fd);
            }

            {
                ...
                // oss << suffix << ".metadata";
                // FILE *fp = fopen(oss.str().c_str(), "wb");
                std::string name = suffix + ".metadata";
                int fd = open(name.c_str(), O_RDWR | O_CREAT, 0644);
                close(fd);
            }
            ...
            size_t total_size = sm->level_capacity[MIN_DISK_LEVEL - MIN_LEVEL - 1];
            #ifdef DETAIL
            std::cout << "shm size: " << total_size << " = " << total_size / BLOCK_SIZE << " Blocks" << std::endl;
            #endif
            if (ftruncate(shm_fd, total_size) == -1) {
                std::cerr << "shared memory has been truncated" << std::endl;
            }
            void *ptr = mmap(nullptr, total_size, PROT_READ | PROT_WRITE, MAP_SHARED, shm_fd, 0);
            if (ptr == MAP_FAILED) {
                std::cerr << "Failed to map shared memory." << std::endl;
                exit(1);
            }
            shm_ptr = static_cast<char*>(ptr);
            return;
        }

        void close_files() {
            for (auto i = 0; i < 12 - min_disk_level; i++) {
                close(data_file_handlers[i]);
            }

            for (auto i = 0; i < 12 - min_index_level; i++) {
                close(index_file_handlers[i]);
            }

            close(metadata_handler);
            close(shm_fd);
        }
        ...
        int get_data_file_handler(uint8_t level) {
            return level >= min_disk_level ? data_file_handlers[level - min_disk_level] : -1;
            // return data_file_handlers[level - min_level];
        }

        int get_index_file_handler(uint8_t level) {
            return index_file_handlers[level - min_index_level];
        }

        void read_block_or_not(uint8_t cur_level, int file_handler, int32_t *last_block,
                               int32_t block_to_fetch, void *block_data) {
            if (block_to_fetch != *last_block) {
                sm->read_block(cur_level, file_handler, block_data, block_to_fetch, shm_ptr);
                *last_block = block_to_fetch;
            }
        }

        void read_block_and_cache(uint8_t cur_level, int file_handler, int32_t *last_block,
                               int32_t block_to_fetch, void *block_data, int save_left, int save_right) {
            sm->read_block_and_cache(cur_level, file_handler, block_data, block_to_fetch, shm_ptr, save_left, save_right, cached_block0, cached_block1);
            *last_block = block_to_fetch;
        }
        ...
        ItemOnDisk obtain_item_on_disk(uint8_t cur_level, size_t pos, int file_handler, int *last_block,
                                       void *block_data) {
            int block_to_fetch = -1;
            int item_offset = -1;
            item_block_offset(pos, &block_to_fetch, &item_offset);
            read_block_or_not(cur_level, file_handler, last_block, block_to_fetch, block_data);
            ItemOnDisk *_it_seg =
                    (ItemOnDisk *)(static_cast<char*>(block_data) + sizeof(ItemOnDisk) * item_offset);
            return {_it_seg->key, _it_seg->value};
        }

        // 从 TEE/Disk 或者从预取缓存中读取
        ItemOnDisk obtain_item_on_disk_or_cache(uint8_t cur_level, size_t pos, int file_handler, int *last_block,
                                       void *block_data) {
            int block_to_fetch = -1;
            int item_offset = -1;
            item_block_offset(pos, &block_to_fetch, &item_offset);
            ...
            if (block_to_fetch != *last_block) {
                if (block_to_fetch == cached_block_id0) {
                    memcpy(block_data, cached_block0, BLOCK_SIZE);
                } else if (block_to_fetch == cached_block_id1) {
                    memcpy(block_data, cached_block1, BLOCK_SIZE);
                } else {
                    sm->read_block(cur_level, file_handler, block_data, block_to_fetch, shm_ptr);
                }
                *last_block = block_to_fetch;
        ...
        ItemOnDisk obtain_item_on_disk_and_save(uint8_t cur_level, size_t pos, int file_handler, int *last_block,
                                       void *block_data, size_t first, size_t last) {
            int block_to_fetch = -1;
            int item_offset = -1;

            int first_block_id, last_block_id;
            // pgm index 保证了 first 和 last 对应的 block id 不会小于 0 或 超出最大块数 (见 search 函数中的 PGM_SUB_EPS 和 PGM_ADD_EPS)
            item_block_offset(first, &first_block_id, &item_offset);
            item_block_offset(last, &last_block_id, &item_offset);

            item_block_offset(pos, &block_to_fetch, &item_offset);

            int save_left = first_block_id < block_to_fetch;
            int save_right = last_block_id > block_to_fetch;
            
            // predict
            // if (count_left > 0) {
            //     double ratio = (double)count_right / count_left;
            //     if (ratio > 1.5)
            //         save_left = 0;
            //     else if (ratio < 0.67)
            //         save_right = 0;
            // }
            if (save_left) cached_block_id0 = block_to_fetch - 1;
            if (save_right) cached_block_id1 = block_to_fetch + 1;
            read_block_and_cache(cur_level, file_handler, last_block, block_to_fetch, block_data, save_left, save_right);
        ...
        bool update_item_on_disk(uint8_t cur_level, size_t pos, ItemOnDisk new_item, int file_handler,
                                 void *block_data) {
            int block_to_fetch = -1;
            int item_offset = -1;
            item_block_offset(pos, &block_to_fetch, &item_offset);
            ((ItemOnDisk *)((char *)block_data + sizeof(ItemOnDisk) * item_offset))
                    ->key = new_item.key;
            ((ItemOnDisk *)((char *)block_data + sizeof(ItemOnDisk) * item_offset))
                    ->value = new_item.value;
            sm->write_block(cur_level, file_handler, block_data, block_to_fetch, shm_ptr);
            return true;
        }

        size_t lower_bound_bl_disk_pgm(uint8_t cur_level, int file_handler, size_t first, size_t last,
                                   const K &x, size_t N, ItemOnDisk *item) {
            int last_block = -1;
            if (first == last) return first;
            auto n = last - first;
            int middle_block, useless_offset;
            item_block_offset(first + n / 2, &middle_block, &useless_offset);

            if (n > 1) {
                auto half = n / 2;
                first = obtain_item_on_disk_and_save(cur_level, first + half, file_handler, &last_block, buf, first, last)
                                .key < x
                        ? first + half
                        : first;
                n -= half;
            ...
            while (n > 1) {
                auto half = n / 2;
                first = obtain_item_on_disk_or_cache(cur_level, first + half, file_handler, &last_block, buf)
                                .key < x
                        ? first + half
                        : first;
                n -= half;
            ...
            size_t pos =
                    first +
                    (obtain_item_on_disk_or_cache(cur_level, first, file_handler, &last_block, buf).key < x);
            if (pos != N) {
                *item = obtain_item_on_disk_or_cache(cur_level, pos, file_handler, &last_block, buf);
            }
            // int correct_block;
            // item_block_offset(pos, &correct_block, &useless_offset);
            // if (correct_block > middle_block)
            //     count_right++;
            // else if (correct_block < middle_block)
            //     count_left++;
            // else
            //     count_correct++;
        ...
        size_t lower_bound_bl_disk(uint8_t cur_level, int file_handler, size_t first, size_t last,
                                   const K &x, size_t N, ItemOnDisk *item) {
            int last_block = -1;
            if (first == last) return first;
            auto n = last - first;
            ...
            while (n > 1) {
                auto half = n / 2;
                first = obtain_item_on_disk(cur_level, first + half, file_handler, &last_block, buf)
                                .key < x
                        ? first + half
                        : first;
                n -= half;
            ...
            size_t pos =
                    first +
                    (obtain_item_on_disk(cur_level, first, file_handler, &last_block, buf).key < x);
            if (pos != N) {
                *item = obtain_item_on_disk(cur_level, pos, file_handler, &last_block, buf);
            }
            #ifdef DETAIL
            int correct_block;
            item_block_offset(pos, &correct_block, &useless_offset);
        ...
        void pairwise_merge_on_disk(ItemOnDisk new_item, uint8_t target,
            ...
            // read the data in min_level
            {
                ...
                for (auto i = 0; i < block_count; i++) {
                    sm->read_block(min_level, get_data_file_handler(min_level), buf, i, shm_ptr);
                    int cp_size = ItemCountPerBlock;
                    if (i == block_count - 1 && item_count_last > 0)
                        cp_size = item_count_last;
                    memcpy(tmp_b + cp_offset, buf, ItemOnDiskSize * cp_size);
                    cp_offset += cp_size;
                }
                ...
                if (insertion_point < item_count)
                    memcpy(tmp_a + insertion_point + 1, tmp_b + insertion_point,
                           ItemOnDiskSize * (item_count - insertion_point));
            }
            ...
            auto alternate = true;
            for (uint8_t i = 1 + min_level; i <= merge_limit;
...

                {
                    ...
                    for (auto bi = 0; bi < block_count; bi++) {
                        sm->read_block(i, get_data_file_handler(i), buf, bi, shm_ptr);
                        int cp_size = ItemCountPerBlock;
                        if (bi == block_count - 1 && item_count_last > 0)
                            cp_size = item_count_last;
                        memcpy(tmp_i + cp_offset, buf, ItemOnDiskSize * cp_size);
                        cp_offset += cp_size;
                    }
            ...
            ItemOnDisk *items = (ItemOnDisk *)buf;
            size_t block = 0;
            size_t start = 0;
            ...
            for (; start < tmp_size;) {
                ...
                sm->write_block(target, get_data_file_handler(target), buf, block, shm_ptr);
            ...
            if (has_pgm(target)) {
                ...
                //            auto item_vec = alternate ? std::vector<ItemOnDisk> (tmp_a,
                //            tmp_a + sizeof(tmp_a))
                //                                      :  std::vector<ItemOnDisk> (tmp_b,
                //                                      tmp_b + sizeof(tmp_b));
                // flush index as needed
                pgm(target) = PGMType(item_vec.begin(), item_vec.end(), inner_disk,
                                      get_index_file_handler(target), epsilon_value, sm);
            }
        ...
        void insert_item_on_file(int level, size_t position, ItemOnDisk item) {
            int file_handler = get_data_file_handler(level);
            ...
            if (position == item_count) {
                if (offset == 0) {  // we just write a block at block_id
                    memcpy(buf, &item, ItemOnDiskSize);
                    sm->write_block(level, file_handler, buf, block_id, shm_ptr);
                } else {  // read the original block and add the new item
                    sm->read_block(level, file_handler, buf, block_id, shm_ptr);
                    ItemOnDisk *temp = (ItemOnDisk *)buf;
                    temp[offset] = item;
                    sm->write_block(level, file_handler, buf, block_id, shm_ptr);
                }
                // delete []block_data;
            ...
            if (item_in_last_block > 0)
                block_end += 1;
            else
                item_in_last_block = ItemCountPerBlock;

            // read all the tuples to be moved
            size_t cp_count = 0;
            for (auto i = block_id; i < block_end; i++) {
                sm->read_block(level, file_handler, buf, i, shm_ptr);
                if (i == block_id) {
                    memcpy(items, buf, ItemOnDiskSize * offset);
                    items[offset] = item;
                    cp_count = offset + 1;
                    int remain = i == block_end - 1 ? item_in_last_block - offset
                                                    : ItemCountPerBlock - offset;
                    memcpy(items + cp_count, static_cast<char*>(buf) + ItemOnDiskSize * offset,
                           ItemOnDiskSize * remain);
                    cp_count += remain;
                    continue;
                }
                memcpy(items + cp_count, buf,
                       ItemOnDiskSize *
                       (i == block_end - 1 ? item_in_last_block : ItemCountPerBlock));
            ...
            ItemOnDisk *to_write_items = (ItemOnDisk *)buf;
            ...
            for (; start < cp_count;) {
                ...
                sm->write_block(level, file_handler, buf, block, shm_ptr);
                start += ItemCountPerBlock;
                block += 1;
            }
        ...
        void insert_on_disk_internal(ItemOnDisk new_item) {
            ItemOnDisk item;
            auto item_count = get_item_count(min_level);
            auto insertion_point =
                    lower_bound_bl_disk(min_level, get_data_file_handler(min_level), 0, item_count,
                                        new_item.key, item_count, &item);
            if (insertion_point != item_count && item.key == new_item.key) {
                // todo: update the value on disk; we hope we do this thing in
                // `lower_bound_bl_disk`
                ...
                for (i = min_level + 1; i < used_levels; ++i) {
                    ...
                    if (slots_required <= slots_left_in_level) break;
                ...
                if (need_new_level) {
                    ...
                    if (i - min_index_level >= int(pgms.size()))
                        pgms.emplace_back(epsilon_value);
    ...
    public:
        void count_report() {
            ...
            // std::cout << "count correct: " << count_correct << std::endl;
            // std::cout << "right : left : correct = 1 : " << (double)count_left / count_right << " : " << (double)count_correct / count_right << std::endl;
        }

        void size_report() {
            std::cout << std::endl << "------------------- size report -------------------" << std::endl;
            std::cout << "buffer max size" << std::endl;
            std::cout << buffer_max_size << std::endl;
            std::cout << "buffer max size in KiB" << std::endl;
            std::cout << buffer_max_size * ItemOnDiskSize / 1024 << " KiB" << std::endl;
            std::cout << "used levels is " << (uint32_t)used_levels << std::endl;
            size_t pgm_index_size = 0;
            for (auto i = min_level; i < used_levels; ++i) {
                if (has_pgm(i)) {
                    std::cout << "level " << (uint32_t)i << " has " << get_item_count(i)
                              << " items and pgm size is " << pgm(i).size_in_bytes() / 1024 << " KiB" << std::endl;
                    pgm_index_size += pgm(i).size_in_bytes();
                }
            }
            std::cout << "pgm index size is " << pgm_index_size / 1024 << " KiB" << std::endl;
            size_t ree_size = 0;
            for (auto i = min_level; i < min_disk_level; ++i) {
                ree_size += get_item_count(i) * ItemOnDiskSize;
                std::cout << "ree level " << (uint32_t)i << " has " << get_item_count(i) << " items" << std::endl;
            }
            std::cout << "ree size is " << ree_size / 1024 << " KiB" << std::endl;
            size_t ree_buffer_size = 0;
            for (auto i = min_level; i < min_disk_level; ++i) {
                ree_buffer_size += max_size(i) * sizeof(K);
            }
            std::cout << "ree buffer size is " << ree_buffer_size / 1024 << " KiB" << std::endl;
            size_t disk_size = 0;
            for (auto i = min_disk_level; i < used_levels; ++i) {
                std::cout << "disk level " << (uint32_t)i << " has " << get_item_count(i) << " items" << std::endl;
                int _temp = get_data_file_handler(i);
                struct stat file_stat;
                auto res = fstat(_temp, &file_stat);
                disk_size += file_stat.st_size;
                std::cout << "data file size is " << file_stat.st_size << "B" << std::endl;
            }
            std::cout << "disk size is " << disk_size / 1024 << " KiB" << std::endl;
        }
        ...
        std::pair<bool, V> find_on_disk_pgm(const K &key, bool detail = false, uint64_t *first_=nullptr, uint64_t *last_=nullptr, uint64_t *pos_=nullptr) {
            ItemOnDisk item;
            for (auto i = min_level; i < used_levels; ++i) {
                auto item_count = get_item_count(i);
                if (item_count == 0) continue;
                ...
                if (has_pgm(i)) {
                    // pgm search on disk
                    auto range = inner_disk ? pgm(i).search_disk(get_index_file_handler(i), key)
                                            : pgm(i).search(key);
                    first = range.lo;
                    last = range.hi;
                    if (detail) {
                    ...
                    pos = lower_bound_bl_disk_pgm(i, get_data_file_handler(i), first, last, key,
                                               item_count, &item);
                } else {
                    // binary search on data file
                    if (detail) {
                    ...
                    pos = lower_bound_bl_disk(i, get_data_file_handler(i), first, last, key,
                                               item_count, &item);
                }
                if (detail)
                    *pos_ = pos;
                ...
                if (pos != item_count && item.key == key) {
                    return std::pair(true, item.value);
                }
            }
            return std::pair(false, 0);
        }

        std::pair<bool, V> find_on_disk(const K &key) {
            ItemOnDisk item;
            for (auto i = min_level; i < used_levels; ++i) {
                auto item_count = get_item_count(i);
                if (item_count == 0) continue;
                ...
                // binary search on data file
                auto pos = lower_bound_bl_disk(i, get_data_file_handler(i), first, last, key,
                                               item_count, &item);
        ...
        bool update_on_disk(const K &key, const V &value) {
            ItemOnDisk item;
            for (auto i = min_level; i < used_levels; ++i) {
                auto item_count = get_item_count(i);
                if (item_count == 0) continue;
                ...
                if (pos != item_count && item.key == key) {
                    ItemOnDisk new_item = {key, value};
                    update_item_on_disk(pos, new_item, get_data_file_handler(i), buf);
                    return true;
                }
            }
            return false;
        ...
        int range_on_disk(const K &lo, const K &hi, int len, 
                          ItemOnDisk *tmp_a, ItemOnDisk *tmp_b, int *fsize,
                          ItemOnDisk *results = nullptr) {
            ...
            for (auto i = min_level; i < used_levels; i++) {
                ...
                if (has_pgm(i)) {
                    auto range = inner_disk ? pgm(i).search_disk(get_index_file_handler(i), lo)
                                            : pgm(i).search(lo);
                    lo_first = range.lo;
                    lo_last = range.hi;
                    ...
                    ItemOnDisk _item;
                    lo_first = lower_bound_bl_disk(i, get_data_file_handler(i), lo_first,
                                                   lo_last, lo, lo_last, &_item);
                }
                ...
                while (l_i <= tmp_size && r_i < item_count_i && !is_out_of_range &&
                       offset < ItemCountPerBlock) {
                    int last_block = -1;
                    read_block_or_not(i, get_data_file_handler(i), &last_block, block_id, buf);
                    ItemOnDisk *item_r = (ItemOnDisk *)(buf);
                    ...
                    while (offset < ItemCountPerBlock && l_i == tmp_size &&
                           r_i < item_count_i && !is_out_of_range) {
                        if (o_i >= len) {
                            is_out_of_range = true;
                            break;
                        }
                ...
                // here, when only to handle the case that the remaining tuples in left
                // table append the remaining from one side
                for (; l_i < tmp_size && (is_out_of_range || r_i == item_count_i);
                       l_i++) {
                    out_it[o_i].key = tmp_it[l_i].key;
                    out_it[o_i].value = tmp_it[l_i].value;
                    o_i += 1;
        ...
        void insert_on_disk(K key, V value) {
            ItemOnDisk x;
            x.key = key;
            x.value = value;
            insert_on_disk_internal(x);
        }
        /// ***** End for disk version *****///

    private:
        const Level &level(uint8_t level) const { return levels[level - min_level]; }
        const PGMType &pgm(uint8_t level) const {
            return pgms[level - min_index_level];
        }
        ...
        void pairwise_merge(const Item &new_item, uint8_t target, size_t size_hint,
            ...
            auto alternate = true;
            auto it =
                    std::move(level(min_level).begin(), insertion_point, tmp_a.begin());
            *it++ = new_item;
            ...
            for (uint8_t i = 1 + min_level; i <= merge_limit;
                ...
                if (can_delete_permanently)
                    out_end = merge<true, true>(tmp_begin, tmp_end, level(i).begin(),
                                                level(i).end(), out_begin);
                else
                    out_end = merge<false, true>(tmp_begin, tmp_end, level(i).begin(),
                                                 level(i).end(), out_begin);
                ...
                level(i).clear();
                if (i >= max_fully_allocated_level()) level(i).shrink_to_fit();
                if (has_pgm(i)) pgm(i) = PGMType(epsilon_value);
            }
            ...
            if (has_pgm(target))
                pgm(target) =
                        PGMType(level(target).begin(), level(target).end(), epsilon_value);
        }

        void insert(const Item &new_item) {
            auto insertion_point = lower_bound_bl(level(min_level).begin(),
                                                  level(min_level).end(), new_item);
            if (insertion_point != level(min_level).end() &&
    ...
    public:
         ...
         * @param buffer_level determines the size of level 0, equal to the sum of
         * base^i for i = 0, ..., buffer_level
         * @param index_level the minimum level at which an index is constructed to
         * speed up searches
         */
        DynamicPGMIndex(size_t eps, uint8_t base = 8, uint8_t buffer_level = MIN_LEVEL,
                              uint8_t index_level = MIN_INDEX)
                : base(base),
                  min_level(buffer_level ? buffer_level
                                         : ceil_log_base(128) - (base == 2)),
                  min_index_level(std::max<size_t>(
                          min_level + 1,
                          index_level ? index_level : ceil_log_base(size_t(1) << 21))),
                  buffer_max_size(),
            ...
            for (auto j = 0; j <= min_level; ++j) buffer_max_size += max_size(j);
        ...
        DynamicPGMIndex(bool is_first_time, bool _inner_disk, Iterator first,
                              Iterator last, size_t eps, std::string suffix_ = "test", uint8_t base = BASE,
                              uint8_t buffer_level = MIN_LEVEL, uint8_t index_level = MIN_INDEX, uint8_t min_disk_level_ = MIN_DISK_LEVEL)
                : DynamicPGMIndex(eps, base, buffer_level, index_level) {
            size_t n = std::distance(first, last);
            suffix = "data/TEEIdx/" + suffix_;
            shm_name = "/" + suffix_ + "_shm";
            min_disk_level = min_disk_level_;
            ...
            #ifdef DETAIL
            std::cout << std::endl << "PGM Params" << std::endl;
            std::cout << "is inner:" << inner_disk << std::endl;

            std::cout << "min_level: " << (uint32_t)min_level << std::endl;
            std::cout << "min_disk_level: " << (uint32_t)min_disk_level << std::endl;
            std::cout << "min_index_level: " << (uint32_t)min_index_level << std::endl;
            std::cout << "buffer_max_size: " << buffer_max_size << std::endl;
            std::cout << "used_levels: " << (uint32_t)used_levels << std::endl;
            #endif
            ...
            if (!is_first_time) {
                ...
                for (auto i = min_level; i < 12; i++) {
                    if (get_item_count(i) > 0 && has_pgm(i)) {
                        pgm(i - min_index_level) =
                                PGMType(get_index_file_handler(i), inner_disk, epsilon_value, sm);
                    }
                }
                return;
            }
            ...
            *out++ = Item(first->first, first->second);
            ...
            for (; start < target.size();) {
                ...
                sm->write_block(used_levels - 1, get_data_file_handler(used_levels - 1), buf, block, shm_ptr);
                // auto end_time = std::chrono::high_resolution_clock::now();
                // write_time += std::chrono::duration_cast<std::chrono::nanoseconds>(end_time - start_time).count();
                start += ItemCountPerBlock;
                block += 1;
            }
            // std::cout << "write time: " << write_time / 1e9 << "sec" << std::endl;
            // delete []block_data;
            ...
            if (has_pgm(used_levels - 1)) {
                ...
                pgm(used_levels - 1) =
                        PGMType(target.begin(), target.end(), inner_disk,
                                get_index_file_handler(used_levels - 1), epsilon_value, sm);
            }
            // auto end_time = std::chrono::high_resolution_clock::now();
            // std::cout << "pgm build time: " << std::chrono::duration_cast<std::chrono::nanoseconds>(end_time - start_time).count() / 1e9 << "sec" << std::endl;
        }

        template <typename Iterator>
        void build(std::string path, bool is_first_time, bool _inner_disk,
                   Iterator first, Iterator last, size_t eps, uint8_t base = 8,
                   uint8_t buffer_level = 0, uint8_t index_level = 0) {
            epsilon_value = eps;
            suffix = path;
            std::cout << "files are stored in " << suffix << std::endl;
            ...
            if (!is_first_time) {
                ...
                for (auto i = min_level; i < 12; i++) {
                    if (get_item_count(i) > 0 && has_pgm(i)) {
                        pgm(i - min_index_level) =
                                PGMType(get_index_file_handler(i), inner_disk, epsilon_value);
                    }
                }
                return;
            }
            ...
            *out++ = Item(first->first, first->second);
            ...
            for (; start < target.size();) {
                ...
                write_block(get_data_file_handler(used_levels - 1), buf, block);
                start += ItemCountPerBlock;
                block += 1;
            }
            // delete []block_data;
            ...
            if (has_pgm(used_levels - 1)) {
                ...
                pgm(used_levels - 1) =
                        PGMType(target.begin(), target.end(), inner_disk,
                                get_index_file_handler(used_levels - 1), epsilon_value);
            }
        }

        /**
         * Inserts an element into the container if @p key does not exists in the
         * container. If @p key already exists, the corresponding value is updated
         * with @p value.
         ...
         * @return an iterator to an element with key equivalent to @p key. If no such
         * element is found, end() is returned
         */
        ...
        /**
         * Returns a copy of the elements with key between and including @p lo and @p
         * hi.
         * @param lo lower endpoint of the range query
         * @param hi upper endpoint of the range query, must be greater than or equal
         * to @p lo
        ...
        std::vector<std::pair<K, V>> range(const K &lo, const K &hi) const {
            ...
            for (auto i = min_level; i < used_levels; ++i) {
                ...
                tmp_size = std::distance(
                        out_it,
                        merge<false, false>(tmp_it, tmp_it + tmp_size, it_lo, it_hi, out_it));
            ...
            for (auto it = first; it != last; ++it)
                if (!it->deleted()) result.emplace_back(it->first, it->second);
            return result;
        }

        /**
         * Returns an iterator pointing to the first element that is not less than
         * (i.e. greater or equal to) @p key.
         * @param key key value to compare the elements to
         * @return an iterator to an element with key not less than @p key. If no such
         * element is found, end() is returned
         */
        iterator lower_bound(const K &key) const {
            ...
            for (auto i = min_level; i < used_levels; ++i) {
                ...
                for (auto it = lower_bound_bl(first, last, key);
                    ...
                    if (it->deleted())
                        ...
                        if (it->first == key) return iterator(this, i, it);
                        lb = it;
                        lb_level = i;
                        lb_set = true;
                        break;
            ...
            if (lb_set) return iterator(this, lb_level, lb);
            return end();
        }
         ...
         */
        iterator end() const {
            return iterator(this, levels.size() - 1, levels.back().end());
        }
         ...
         */
        size_t size() const {
            // TODO: scanning the levels and using a hash table for the encountered keys
        ...
        size_t size_in_bytes() const {
            ...
            for (auto &l : levels) bytes += l.size() * sizeof(Item);
        ...
        size_t index_size_in_bytes() const {
            size_t bytes = 0;
            for (auto &p : pgms) bytes += p.size_in_bytes();
            return bytes;
        }

    private:
        template <bool SkipDeleted, bool Move, typename In1, typename In2,
                typename OutIterator>
        static OutIterator merge(In1 first1, In1 last1, In2 first2, In2 last2,
                                 OutIterator result) {
...
/* LoserTree implementation adapted from Timo Bingmann's https://tlx.github.io
 * and http://stxxl.org, and from Johannes Singler's
 * http://algo2.iti.uni-karlsruhe.de/singler/mcstl. These three libraries are
 * distributed under the Boost Software License 1.0. */
        ...
        class LoserTree {
            ...
            std::vector<Loser>
                    losers;  ///< Vector of size 2k containing loser tree nodes.

            static uint64_t next_pow2(uint64_t x) {
                return x == 1 ? 1
                              : uint64_t(1) << (sizeof(unsigned long long) * 8 -
                                                __builtin_clzll(x - 1));
            }
    ...
    class DynamicPGMIndex<K, V, PGMType>::Iterator {
        ...
        struct Cursor {
            ...
            Cursor() = default;
            Cursor(uint8_t level_number, const level_iterator iterator)
                    : level_number(level_number), iterator(iterator) {}
        };

        const dynamic_pgm_type
                *super;        ///< Pointer to the container that is being iterated.
        Cursor current;    ///< Pair (level number, iterator to the current element).
        bool initialized;  ///< true iff the members tree and iterators have been
        ///< initialized.
        uint8_t unconsumed_count;  ///< Number of iterators that have not yet reached
        ///< the end.
        internal::LoserTree<K>
                tree;  ///< Tournament tree with one leaf for each iterator.
        std::vector<Cursor>
                iterators;  ///< Vector with pairs (level number, iterator).
        ...
        void lazy_initialize() {
            ...
            for (uint8_t i = super->min_level; i < super->used_levels; ++i) {
                ...
                auto pos = std::upper_bound(level.begin() + lo, level.begin() + hi,
                                            current.iterator->first);
                if (pos != level.end()) iterators.emplace_back(i, pos);
            }
        ...
        void advance() {
            ...
            do {
                tmp = step();
                while (unconsumed_count > 0 &&
                       iterators[tree.min_source()].iterator->first ==
                       tmp.iterator->first)
                    step();
        ...
        Iterator(const dynamic_pgm_type *p, uint8_t level_number,
                 const level_iterator it)
                : super(p),
                  current(level_number, it),
                  initialized(),
                  unconsumed_count(),
                  tree(),
                  iterators(){};
    ...
    public:
        ...
        bool operator==(const Iterator &rhs) const {
            return current.level_number == rhs.current.level_number &&
                   current.iterator == rhs.current.iterator;
        }
...
#pragma pack(push, 1)
    ...
    class DynamicPGMIndex<K, V, PGMType>::ItemA {
        ...
        static V get_tombstone() {
            return new std::remove_pointer_t<V>();
        }
    ...
    public:
        ...
        ItemA() { /* do not (default-)initialize for a more efficient
               std::vector<ItemA>::resize */
        }
        ...
        explicit ItemA(const K &key, const V &value) : first(key), second(value) {
            if (second == tombstone)
                throw std::invalid_argument(
                        "The specified value is reserved and cannot be used.");
        }
    ...
    template <typename K, typename V, typename PGMType>
    const V DynamicPGMIndex<K, V, PGMType>::ItemA::tombstone =
            get_tombstone<V>();
    ...
    public:
        ...
        explicit ItemB(const K &key, const V &value)
                : flag(false), first(key), second(value) {}
