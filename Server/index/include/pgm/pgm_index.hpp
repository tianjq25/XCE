...
#ifndef PGM_INDEX_BASE_HPP
#define PGM_INDEX_BASE_HPP
#pragma once
 ...
 */
    ...
    protected:
        ...
        struct Segment;
        void *buf = aligned_alloc(BLOCK_SIZE, 100 * BLOCK_SIZE);
        ...
        static void build(RandomIt first, RandomIt last,
            ...
            // std::cout << "leaf segs " << levels_offsets[1] << std::endl;
        }
        ...
        void read_block_or_not(int file_handler, int32_t *last_block, int32_t block_to_fetch, void *block_data) {
            if (block_to_fetch != *last_block) {
                sm->read_block(file_handler, block_data, block_to_fetch);
        ...
        size_t predict_pos(size_t it, int file_handler, int *last_block, void *buf, K key) {
            int block_to_fetch = -1;
            int item_offset = -1;
            item_block_offset(it, &block_to_fetch, &item_offset);
            read_block_or_not(file_handler, last_block, block_to_fetch, buf);
//        if (ACCESSED_BLOCK_COUNT_Index == 4) {
//            int x = 0;
//        }
            Segment *it_seg = (Segment *)(static_cast<char*>(buf) + sizeof(Segment) * item_offset);
            ...
            Segment *it_next_seg = (Segment *)(static_cast<char*>(buf) + sizeof(Segment) * next_offset);

            size_t pos = std::min<size_t>(est_pos, it_next_seg->intercept);
            return pos;
        }

        K obtain_key_from_disk(size_t it, int file_handler, int *last_block, void *block_data) {
            int block_to_fetch = -1;
            int item_offset = -1;
            item_block_offset(it, &block_to_fetch, &item_offset);
            read_block_or_not(file_handler, last_block, block_to_fetch, block_data);
            Segment *_it_seg = (Segment *)(static_cast<char*>(block_data) + sizeof(Segment) * item_offset);
            return _it_seg->key;
        }

        size_t segment_for_key_disk(int file_handler, const K &key) {
            // do not consider the case `EpsilonRecursive == 0'
            int last_block = -1;
            // char *block_data = new char[BLOCK_SIZE];
            size_t it = *(levels_offsets.end() - 2);
            for (auto l = int(height() -2); l >= 0; l--) {
                auto pos = predict_pos(it, file_handler, &last_block, buf, key);
                ...
                if constexpr (EpsilonRecursive <= linear_search_threshold) {
                    for (;;++lo) {
                        auto lo_next_key = obtain_key_from_disk(lo + 1, file_handler, &last_block, buf);
                    ...
                    while (count > 0) {
                        auto step = count / 2;
                        auto _it_ = lo + step;
                        auto _it_key = obtain_key_from_disk(_it_, file_handler, &last_block, buf);
                        if (!(key < _it_key)) {
                            lo = _it_ + 1;
                            count -= step + 1;
                        } else count = step;
    ...
    public:
         ...
         */
        PGMIndex() = default;
        PGMIndex(size_t eps):epsilon_value(eps){};

        /**
         * Constructs the index on the given sorted vector.
         * @param data the vector of keys to be indexed, must be sorted
         */
        explicit PGMIndex(const std::vector<K> &data, size_t eps) : PGMIndex(data.begin(), data.end(), eps) {}
         ...
         */
        template<typename RandomIt>
        PGMIndex(RandomIt first, RandomIt last, size_t eps)
                : n(std::distance(first, last)),
                  first_key(n ? *first : K(0)),
                  segments(),
                  levels_offsets(),
                  epsilon_value(eps) {
            build(first, last, epsilon_value, EpsilonRecursive, segments, levels_offsets);
        }

        template<typename RandomIt>
        PGMIndex(RandomIt first, RandomIt last, bool _inner_disk, int file_handler, int eps, StorageManager *storage_manager)
                : n(std::distance(first, last)),
                  first_key(n ? *first : K(0)),
                  segments(),
                  levels_offsets(),
            ...
            // std::cout << "build time: " << std::chrono::duration_cast<std::chrono::nanoseconds>(end_time - begin_time).count() / 1e9 << "sec" << std::endl;
            if (_inner_disk) {
                write_file(file_handler);
            }
        }

        PGMIndex(int file_handler, bool inner_disk, size_t eps, StorageManager *storage_manager)
                : sm(storage_manager), epsilon_value(eps) {
            load_file(file_handler, inner_disk);
        }

        void load_file(int file_handler, bool inner_disk) {
            // char *block_data = new char[BLOCK_SIZE];
            sm->read_block(file_handler, buf, 0);
            int offset = 0;
            memcpy(&first_key, static_cast<char*>(buf) + offset, sizeof(K));
            offset += sizeof(K);
            memcpy(&n, static_cast<char*>(buf) + offset, sizeof(size_t));
            offset += sizeof(size_t);
            size_t level_size;
            memcpy(&level_size, static_cast<char*>(buf) + offset, sizeof(size_t));
            offset += sizeof(size_t);
            size_t seg_size;
            memcpy(&seg_size, static_cast<char*>(buf) + offset, sizeof(size_t));
            offset += sizeof(size_t);
            levels_offsets = decltype(levels_offsets)(level_size);
            size_t *p = (size_t *)(static_cast<char*>(buf) + offset);
        ...
        // todo: implement a disk version
        ApproxPos search_disk(int file_handler, const K&key) {
            auto k = std::max(first_key, key);
            // on disk version, we calculate the pos
            auto pos = segment_for_key_disk(file_handler, key);
            return {pos, PGM_SUB_EPS(pos, epsilon_value), PGM_ADD_EPS(pos, epsilon_value, n)};
        }

        void write_file(int file) {
            int32_t block = 0;
            // char *block_data = new char[BLOCK_SIZE];
            memcpy(buf, &first_key, sizeof(K));
            // write offsets info
            size_t *pointer = (size_t *)(static_cast<char*>(buf) + sizeof(K));
            ...
            sm->write_block(file, buf, block);
            block += 1;

            // write segments info
            int32_t start = 0;
            Segment *items = (Segment *)buf;
