...
#include "./buffer/tee_buffer_manager.h"
#include <cstdint>
#include <limits> 
#include <stdio.h>
#include <iostream>
...
class BTree {
    ...
    private:
        ...
        void init_metanode() {
            ...
            mn.root_block_id = 1;
            mn.level = 1;
            memcpy(temp_block.data, &mn, MetaNodeSize);
            bm->write_block(0, temp_block);
        }
...
public:
    ...
    size_t get_file_size() {
            return bm->get_file_size();
        }
    void sync_metanode() {
        Block block;
        memcpy(block.data, &metanode, MetaNodeSize);
        bm->write_block(0, block);
    }
        BTree(bool first, char *file_name, bool bulk_load =false) {
            bm = new TEEBufferManager(file_name, first, bulk_load);
            load_metanode();
        }
        ...
        BTree(int _hybrid_mode, bool first, char *main_file, bool bulk_load = false, char *_utility_file = nullptr) {
            ...
            bm = new TEEBufferManager(main_file, first, bulk_load);
            init_metanode();
            load_metanode();
        ...
        InnerNodeIterm split_node(int POS, int block_id, Block block_to_split, NodeIterm *new_entry, bool is_right_most_inner = false) {
            ...
            bm->write_block(block_id, block_to_split);
            ...
            bm->write_block(metanode.block_count, block_r);
...
//            sync_metanode();
        ...
        void add_entry_in_node(int POS, int block_id, Block block_to_append, NodeIterm *ini, bool is_right_most_inner = false) {
            ...
            if (POS > -1 && !is_right_most_inner) {
                ...
                bm->write_block(block_id, block_to_append);
                return;
            }
    ...
    int SMO_UPDATE = 0;
        ...
        BuildStatus insert_key(int current_block_id, LeafNodeIterm lni_item) {
            ...
            BuildStatus bs;
            Block block;
            // std::cout << "get block with current_block_id: " << current_block_id << std::endl;
            block = bm->get_block(current_block_id);
            // std::cout << "get block done" << std::endl;
            bool is_inner = block.data[0] == InnerNodeType;
            // std::cout << "is_inner" << is_inner << std::endl;
            if (is_inner) {
        ...
        int insert_key_leaf_disk(LeafNodeIterm lni) {
            ...
            // do insert
            Block block;
            bm->get_block(block_id, block.data);
...
//                    inner_btree.insert(indexed_key, to_add_iterm.block_id);
        ...
        int insert_key_disk(LeafNodeIterm lni) {
            ...
            // std::cout << "metanode.root_block_id: " << metanode.root_block_id << std::endl;
            ...
            // std::cout << "new entry" << std::endl;
                ...
                bm->write_block(metanode.block_count, block);
...
//                sync_metanode();
        ...
        int insert_key_entry(KeyType key, ValueType value, long long *search_time, long long *smo_time,
                             long long *insert_time, int *smo_c, int *update_c, uint64_t &IO_time) {
#if Profiling
            SEARCH_USED_TIME = 0;
            SMO_USED_TIME = 0;
            INSERT_USED_TIME = 0;
        ...
        class IndexIterator {
            ...
            public:
                ...
                IndexIterator(LeaftNodeHeader *start_header, LeafNodeIterm *start_iterms, int start_point, Condition _cond, TEEBufferManager *_bm) {
                ...
                IndexIterator(Condition _cond, TEEBufferManager *_bm) {
                ...
                bool has_next() {
                    if (next_point == current_leaf_node_header->item_count) {
                        bm->get_block(current_leaf_node_header->next_block_id, data);
                        next_point = 0;
                    }
        ...
        std::pair<bool, ValueType> lookup_disk(Condition cond, int *c) {
            IndexIterator idx_it = get_index_iterator(cond, c);
            bool found = false;
            while (idx_it.has_next(c)) {
                return {true, idx_it.next()};
                found = true;
            }
            return {found, idx_it.next()};
        }

        std::pair<bool, ValueType> bookup_leaf_disk(Condition cond, int *c) {
            ...
            return {found, ii.next()};
        }

        std::pair<bool, ValueType> lookup(KeyType key, int *c = new int()) {
            Condition cond;
        ...
        bool scan_disk(ValueType *results, Condition cond, int len, int *c) {
            ...
            // std::cout << "metanode.root_block_id after scan_disk: " << metanode.root_block_id << std::endl;
            return found;
        }
        ...
        bool scan(ValueType *results, KeyType lower_key, int len, int *c, uint64_t &IO_time) {
            Condition cond;
            ...
            *c = 0;
            bm->reset_IO_time();
            bool found;
            // std::cout << "scan metanode.root_block_id: " << metanode.root_block_id << std::endl;
            if (hybrid_mode == ALL_DISK) {
                found = scan_disk(results, cond, len, c);
            } else if (hybrid_mode == LEAF_DISK) {
                found = scan_leaf_disk(results, cond, len, c);
            }
            IO_time = bm->get_IO_time();
            return found;
        }
        ...
        void bulk_load(LeafNodeIterm *data, long item_count, double per=0.8) {
            ...
            // std::cout << "leaf node count " << leaf_node_count << std::endl;
            ...
            for (int i = 0; i < leaf_node_count; i++) {
                ...
                // if (i < 10)
                //     std::cout << "inis[" << i << "].block_id: " << inis[i].block_id << std::endl;
...
//                    sync_metanode();
            ...
            // std::cout << "level num " << metanode.level << std::endl;
            metanode.root_block_id = inis[0].block_id;
            // std::cout << "root block id " << metanode.root_block_id << std::endl;
            delete []inis;
        }
